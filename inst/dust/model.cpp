...
public:
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    real_type trateh = dust::math::max(static_cast<real_type>(0), beta_next * MIh * shared->N * hp(1) * S_vacc_use);
    ...
    real_type tratef = dust::math::max(static_cast<real_type>(0), MSIf_vacc * shared->N * fp(1) * rf);
    real_type trateg = dust::math::max(static_cast<real_type>(0), MSIg_vacc * shared->N * gp(1) * rg);
    ...
    state_next[12] = dust::math::max(static_cast<real_type>(1.0000000000000001e-09), dust::math::min(static_cast<real_type>(1), thetah + dthetah));
    ...
    state_next[0] = dust::math::max(static_cast<real_type>(1.0000000000000001e-09), dust::math::min(static_cast<real_type>(1), thetaf + dthetaf));
    state_next[6] = dust::math::max(static_cast<real_type>(1.0000000000000001e-09), dust::math::min(static_cast<real_type>(1), thetag + dthetag));
    ...
    real_type dMSSf = 1 * shared->etaf * dust::math::pow(MSf, 2) - shared->etaf * MSSf_vacc - (- dSf) * (delta_si_f / (real_type) fp(1)) * MSSf_vacc / (real_type) MSf - ((- dSg) + (- dSh)) * (dot_thetaf * fp(dot_thetaf) / (real_type) f(dot_thetaf) / (real_type) fp(1)) * MSSf_vacc / (real_type) MSf;
    real_type dMSSg = 1 * shared->etag * dust::math::pow(MSg, 2) - shared->etag * MSSg_vacc - (- dSg) * (delta_si_g / (real_type) gp(1)) * MSSg_vacc / (real_type) MSg - ((- dSf) + (- dSh)) * (dot_thetag * gp(dot_thetag) / (real_type) g(dot_thetag) / (real_type) gp(1)) * MSSg_vacc / (real_type) MSg;
...
dust::pars_type<model> dust_pars<model>(cpp11::list user) {
  ...
  shared->vacc_amt = static_cast<real_type>(0.65000000000000002) * 50000 / (real_type) shared->vacc_duration;
